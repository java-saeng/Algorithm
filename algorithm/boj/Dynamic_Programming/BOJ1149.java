...
public class BOJ1149 {
    public static void main(String args[]) throws IOException{
        ...
        for(int i = 1; i < size; i++){
            arr[i][0] = arr[i][0] + boj.Math.min(arr[i-1][1], arr[i-1][2]);
            arr[i][1] = arr[i][1] + boj.Math.min(arr[i-1][0], arr[i-1][2]);
            arr[i][2] = arr[i][2] + boj.Math.min(arr[i-1][1], arr[i-1][0]);
        }

        int min = 0;
        min = boj.Math.min(arr[size-1][0], boj.Math.min(arr[size-1][1], arr[size-1][2]));
